...
// Actualisation automatique des données à chaque clic sur un onglet/bouton de nav,
// pour ne plus avoir besoin d'un bouton "Actualiser" sur chaque page.
const PAGE_REFRESHERS = {
  info: () => {
    if ((DATA.sheets?.info || '').trim())       loadInfoFromSheet();
    if ((DATA.sheets?.bandeau || '').trim())     loadBandeauFromSheet();
    if ((DATA.sheets?.actualites || '').trim())  chargerActualites();
  },
  accueil: () => {
    loadFromSheets();
  ...
  secondaires:  () => loadFromSheets(),
  'sec-prives': () => loadFromSheets(),
  'sec-publics':() => loadFromSheets(),
  inspections:  () => { if ((DATA.sheets?.ecoles_iepp || '').trim()) loadEcolesIEPP(); },
  admin:        () => { if ((DATA.sheets?.admin || '').trim())       loadAdminFromSheet(); },
  courriers:    () => { if ((DATA.sheets?.courriers || '').trim())   loadCourriers(); },
};

const _lastPageRefresh = {};
const PAGE_REFRESH_MIN_INTERVAL = 4000; // évite de re-fetch si l'utilisateur clique plusieurs fois de suite (ms)

function refreshPageData(name) {
  const fn = PAGE_REFRESHERS[name];
  if (!fn) return;
  const now = Date.now();
  if (_lastPageRefresh[name] && (now - _lastPageRefresh[name]) < PAGE_REFRESH_MIN_INTERVAL) return;
  _lastPageRefresh[name] = now;
  try { fn(); } catch (e) { console.error('Erreur actualisation page', name, e); }
}
...
// Depuis un sheet UNIQUE (DATA.sheets.secondaires), onglet "Réponses au formulaire 1".
// Colonne A (Horodateur) est ignorée, ligne 1 (en-tête) ignorée automatiquement par gviz.
// Colonne B (Type d'établissement) sert à ranger chaque ligne dans "Privés" ou "Publics".
const COL_SEC = {
...
// Toute saisie utilisateur est forcée en MAJUSCULES à l'affichage,
// quelle que soit la casse utilisée dans le formulaire.
function _upper(v) {
  return (v || '').toString().trim().toUpperCase();
}

// Normalise la colonne "Type d'établissement" -> 'prive' | 'public' | null (valeur non reconnue)
function _normType(raw) {
  const t = _upper(raw).normalize('NFD').replace(/[\u0300-\u036f]/g, ''); // retire les accents
  if (t.startsWith('PRIV')) return 'prive';
  if (t.startsWith('PUB'))  return 'public';
  return null;
}
...
function _buildSecEntry(row, id, numero, extra) {
  ...
  return Object.assign({
    id, numero,
    code:         _upper(row[COL_SEC.code]),
    nom:          _upper(row[COL_SEC.nom]),
    chef_nom:     _upper(row[COL_SEC.chef_nom]),
    chef_contact: _upper(row[COL_SEC.chef_contact]),
    profs,
  }, extra || {});
}
...
async function loadFromSheets() {
  ...
  // ── Mode unique : un seul sheet, trié via la colonne "Type d'établissement" ──
  if (sheets.secondaires) {
    try {
      const raw   = await fetch(_sheetUrl(sheets.secondaires, 'Réponses au formulaire 1')).then(r => r.text());
      const table = _parseSheet(raw).table;
      const rows  = _rowsFromTable(table).filter(r => r[COL_SEC.nom]); // colonne A + ligne d'en-tête déjà exclues par gviz

      // Dédoublonnage : nom + type → la ligne la plus récente écrase la précédente
      const mapPrives  = new Map();
      const mapPublics = new Map();

      rows.forEach(r => {
        const type = _normType(r[COL_SEC.type]);
        if (!type) return; // type non reconnu (ni Privé ni Public) → ligne ignorée
        const key = _upper(r[COL_SEC.nom]);
        if (!key) return;
        if (type === 'prive') {
          mapPrives.set(key, _buildSecEntry(r, 0, 0, {}));
        } else {
          mapPublics.set(key, _buildSecEntry(r, 0, 0, { sous_type: 'Collège Moderne', badge_class: 'public' }));
        }
      });

      DATA.secondaires_prives  = Array.from(mapPrives.values()).map((e, i) => ({ ...e, id: i + 1, numero: i + 1 }));
      DATA.secondaires_publics = Array.from(mapPublics.values()).map((e, i) => ({ ...e, id: 50 + i, numero: i + 1 }));
  ...
  // ── Rétrocompatibilité : ancien mode à deux sheets séparés ──
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Toutes les données viennent d'être chargées ci-dessus : on marque toutes les pages
  // comme "fraîchement actualisées" pour éviter un double-fetch immédiat via showPage().
  const _now = Date.now();
  Object.keys(PAGE_REFRESHERS).forEach(p => { _lastPageRefresh[p] = _now; });
...
const DASHBOARD = {
  ...
  renderSkeleton() {
    const kpiHtml = ['Écoles','Élèves','Classes','Enseignants'].map(label => `
      <div class="bg-white p-6 rounded-2xl shadow border border-slate-100 animate-pulse">
  ...
  async fetchSheet(sheetId, sheetName) {
  ...
  async update() {
    ...
    dot.className = 'w-2 h-2 bg-amber-500 rounded-full mr-2 animate-pulse';
    txt.textContent = 'Synchro en cours...';

    try {
      const ecoles = await this.fetchSheet(DATA.sheets.ecoles_iepp, DATA.sheets.ecoles_iepp_onglet);
      const stats = this.computeStats(ecoles);
      this.renderKPI(stats);
      this.renderCharts(stats);
      
      dot.className = 'w-2 h-2 bg-green-500 rounded-full mr-2';
      txt.textContent = `MAJ : ${new Date().toLocaleTimeString('fr-FR')}`;
    } catch(e) {
  ...
  computeStats(rows) {
    const cols = DATA.sheets.ecoles_iepp_colonnes;
    const idx = { 
      type: 2, iepp: 0, eleves: 6, classes: 5 // Ajuste selon ton Google Sheet
    };
    
    let totalEcoles = rows.length;
    let totalEleves = 0, totalClasses = 0;
    let publicCount = 0, priveCount = 0;
    let ieppData = { 'IEPP 1': 0, 'IEPP 2': 0, 'IEPP 3': 0 };

    rows.forEach(r => {
      totalEleves += parseInt(r[idx.eleves]) || 0;
      totalClasses += parseInt(r[idx.classes]) || 0;
      if(String(r[idx.type]).toLowerCase().includes('public')) publicCount++;
      else priveCount++;
      const iepp = r[idx.iepp];
      if(ieppData[iepp] !== undefined) ieppData[iepp]++;
    });

    return { totalEcoles, totalEleves, totalClasses, publicCount, priveCount, ieppData };
  },

  renderKPI(s) {
    const kpis = [
      { label: 'Écoles actives', value: s.totalEcoles, icon: '🏫' },
      { label: 'Élèves', value: s.totalEleves.toLocaleString('fr-FR'), icon: '👥' },
      { label: 'Classes', value: s.totalClasses, icon: '📚' },
      { label: 'Enseignants', value: '~', icon: '👨‍🏫' }
    ];
    document.getElementById('kpi-container').innerHTML = kpis.map(k => `
      <div class="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 hover:shadow-xl transition">
        <div class="text-3xl mb-2">${k.icon}</div>
        <div class="text-3xl font-bold text-slate-900">${k.value}</div>
  ...
  renderCharts(s) {
    ...
    this.charts.pp = new Chart(document.getElementById('chart-public-prive'), {
      type: 'doughnut',
      data: {
        labels: ['Public', 'Privé'],
        datasets: [{ data: [s.publicCount, s.priveCount], backgroundColor: ['#2196F3','#F59E0B'] }]
      },
      options: { responsive: true, plugins: { legend: { position: 'bottom' } } }
    });

    // IEPP
    if(this.charts.iepp) this.charts.iepp.destroy();
    this.charts.iepp = new Chart(document.getElementById('chart-iepp'), {
      type: 'bar',
      data: {
        labels: Object.keys(s.ieppData),
        datasets: [{ label: 'Écoles', data: Object.values(s.ieppData), backgroundColor: '#7C3AED' }]
      },
...
document.addEventListener('DOMContentLoaded', () => {
  if(document.getElementById('dashboard-live')) DASHBOARD.init();
});
